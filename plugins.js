...
if ($(touch.target).is(".mapIcon") || $(touch.target).is(".tool-tip *"))
	...
	$.fn.jRating = function(op) {
		...
		return this.each(function() {
			...
			function getStarWidth(){
				switch(opts.type) {
					case 'small' :
						starWidth = 11; // width of the picture small.png
  	...
  	//doload Boolean checks to see whether to load next section (true if bypasshooks param is true or onpagechangestart() event handler doesn't return false)
		...
		if (this.currentsection!=i && i<this.sections.count && doload){ //if next section to show isn't the same as the current section shown
			...
			if (i==0) //hide "prev" link
				...
				this.paginatediv.$navlinks.eq(1).css('visibility', 'visible')
				}
	...
	init:function(setting){
		...
		jQuery(function($){ //on document.ready
			...
			if (setting.revealfx[0]=="slide"){ //create outer DIV that will house all the fieldset.sectionwrap elements
				$sectionswrapper=$('<div style="position:relative;min-width:950px;overflow:hidden;"></div>').insertBefore($sections.eq(0)) //add DIV above the first fieldset.sectionwrap element
			...
			$sections.each(function(i){ //loop through $sections again
				...
				$thestep.click(function(){ //assign behavior to each step div /Nishith Step '+(i+1)+'
			...
			var $paginatediv=$('<div class="formpaginate" style="overflow:hidden;"><span class="prev" style="float:left">Back</span> <span class="status"> </span>  <span class="save" id="saveAppObj" style="float:right">Save</span> <span class="next" style="float:right">Next</span> </div>')
			...
			thiswizard.paginatediv={$main:$paginatediv, $navlinks:$paginatediv.find('span.prev,span.save, span.next '), $status:$paginatediv.find('span.status')} //remember various parts of pagination DIV
